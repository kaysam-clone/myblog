...
def home(request):
    ...
    post2= Post.objects.filter(publish=True).order_by('-slug')
    post3= Post.objects.filter(publish=True).order_by('-id')
...
def delete_post(request, slug):
    ...
    if request.method == "POST":
        post.delete()
        messages.success(request,f'{post.user} Post Deleted')
        return redirect('/post')
